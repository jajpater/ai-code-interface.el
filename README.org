...
By default, AI Code stores prompt and task files in =.ai.code.files/= under
the main Git repository root.  You can customize
=ai-code-files-directory-strategy= when you want a different location:

#+begin_src elisp
;; Default behavior: .ai.code.files under the Git root.
(setq ai-code-files-directory-strategy 'git-root)

;; Store .ai.code.files under the current buffer's directory.
(setq ai-code-files-directory-strategy 'current-directory)

;; Store files in an explicit directory.
(setq ai-code-files-directory-strategy "~/ai-code-files/")
#+end_src
