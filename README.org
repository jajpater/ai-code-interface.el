...
- *Sending Free-form Prompts*: Press `C-c a`, then `<SPC>` (`ai-code-send-command`) to send a plain prompt.  If prefix arguments are inconvenient in your Emacs setup, use `M-x ai-code-send-command-with-files-context` for files/repo context or `M-x ai-code-send-command-with-clipboard-context` for files/repo/clipboard context.
