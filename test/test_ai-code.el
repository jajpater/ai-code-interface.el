...
(ert-deftest ai-code-test-send-command-with-files-context-appends-repo-context ()
  "Test explicit files-context send command avoids prefix arguments."
  (let (read-args sent-prompt)
    (cl-letf (((symbol-function 'ai-code-read-string)
              ...
              ((symbol-function 'ai-code--get-clipboard-text)
               (lambda ()
                 (ert-fail "Should not read clipboard for files-context command")))
              ((symbol-function 'ai-code--insert-prompt)
               (lambda (prompt)
                 (setq sent-prompt prompt))))
      (ai-code-send-command-with-files-context)
      (should (equal read-args
                     '("Send to AI (files/repo context): " nil)))
      (should (string-match-p "Summarize this" sent-prompt))
      (should (string-match-p "Files:" sent-prompt))
      (should (string-match-p "Repo context:" sent-prompt))
      (should-not (string-match-p "Clipboard context:" sent-prompt)))))

(ert-deftest ai-code-test-send-command-with-clipboard-context-appends-clipboard ()
  "Test explicit clipboard-context send command avoids double prefix arguments."
  (let (read-args sent-prompt)
    (cl-letf (((symbol-function 'ai-code-read-string)
               (lambda (prompt &optional initial-input _candidate-list)
                 (setq read-args (list prompt initial-input))
              ...
              ((symbol-function 'ai-code--format-repo-context-info)
               (lambda () "\nRepo context:\n- current task"))
              ((symbol-function 'ai-code--get-clipboard-text)
               (lambda () "clipboard data"))
              ((symbol-function 'ai-code--insert-prompt)
               (lambda (prompt)
                 (setq sent-prompt prompt))))
      (ai-code-send-command-with-clipboard-context)
      (should (equal read-args
                     '("Send to AI (files/repo/clipboard context): " nil)))
      (should (string-match-p "Summarize this" sent-prompt))
      (should (string-match-p "Files:" sent-prompt))
      (should (string-match-p "Repo context:" sent-prompt))
      (should (string-match-p "Clipboard context:" sent-prompt))
      (should (string-match-p "clipboard data" sent-prompt)))))
